...
const orderList = async (req, res, next) => {
    ...
    let where = `id = ${req.body.id} or status = ${req.body.status} or createTime between ${req.body.startTime} and ${req.body.endTime} `
    let sqlStr = `select * from order where ${where} limit ${(page - 1) * size}, ${size}`
    mysql.query(sqlStr, (err, result) => {
        if (err) {
            res.send({
                code: 500,
